...
export default function Home() {
  ...
  return (
    <div style={{ padding: 20, fontFamily: 'Arial, sans-serif', maxWidth: 900, margin: 'auto' }}>
      ...
      <h1 style={{ fontSize: 28, fontWeight: 'bold', marginBottom: 10 }}>Camisetas Retro Personalizadas</h1>
      <p>Elige un estilo para tu camiseta:</p>
      ...
      {formVisible && selectedDesign && (
        <form onSubmit={handleSubmit} style={{ marginTop: 30, padding: 20, border: '1px solid #ccc', borderRadius: 10 }}>
          <h3>Encargar camiseta seleccionada</h3>
          <p>Introduce tus datos y te contactaremos:</p>
          ...
          <textarea placeholder="Mensaje (talla, equipo, jugador...)" required value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} style={{ width: '100%', padding: 8, marginBottom: 10 }} />
          <button type="submit" style={{ padding: 10, backgroundColor: '#28a745', color: 'white', border: 'none', borderRadius: 5 }}>Enviar pedido</button>
        </form>
      )}
